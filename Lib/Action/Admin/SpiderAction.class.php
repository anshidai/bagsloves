...
class SpiderAction extends Action {
	...
	public function init()
	{
		...
		for($i=1; $i<=$max_id; $i++) {
			$info = M('CjDetail')->where("id={$i} AND status=2 and site_name='2015destinationbags'")->find();
			if(empty($info) || $info['is_push'] == '1') continue;
						
			$related_ids = $info['related_id']? $info['related_id']: $info['goods_id'];
			$relate_goods = M('CjDetail')->where("goods_id in($related_ids) AND status=2 and site_name='2015destinationbags' AND is_push=0")->order('id')->select();
			if($relate_goods) {
				$insert_id = array();
