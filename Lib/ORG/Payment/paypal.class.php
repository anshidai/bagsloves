...
class paypal {
	...
	function paypal() {
		...
		//$this->submit_url = 'http://www.the-boot-shoes.com/'.U('Payment/paypal_c');
		
		//$this->submit_url = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
		//$this->submit_url = 'http://chinagoodmall.com/Home-PaypalProxy-submit-sitename-demo';
	...
	function submit_paypal_post() {
		...
		echo "</form>\n";
		echo "<script>function jump(){ document.forms[\"paypal_form\"].submit();} setTimeout(\"jump()\", 5000);</script>";
		echo "</body></html>\n";

	}
	function submit_paypal_post_c() {
		echo "<html>\n";
		echo "<head><title>Processing Payment...</title><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"></head>\n";
		echo "<body onLoad=\"document.forms['paypal_form'].submit();\">\n";
		echo "<form method=\"post\" name=\"paypal_form\" ";
	...
	function validate_ipn() {
		...
		$url_parsed=parse_url($this->submit_url);
	...
	function create_form($list) {
		...
		$this->add_field ( 'business', GetValue (  $pname. "_account" ) ); //收款人账号'402660_1224487424_biz@qq.com'
		$this->add_field ( 'return', $this_script );//不填的话，按网站设置返回
		$this->add_field ( 'cancel_return', $this_script  );
		$this->add_field ( 'notify_url', $this_script . U ( 'Payment/Pin', array ("type" => "Paypal" ) ) );
		$this->add_field ( 'item_name', 'Products For SN:' . $list ['sn'] ); //产品名称
		$this->add_field ( 'item_number', $list ['sn'] ); //订单号码
		$this->add_field ( 'amount', $list ['orders_total'] ); //交易价格
		$this->add_field ( 'currency_code', $_SESSION ['currency'] ['symbol'] ? $_SESSION ['currency'] ['symbol'] : 'USD' ); //货币代码
